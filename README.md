...
- **Paper trading** — **378 profiles** (3 model strategies × 14 trade strategies × 9 lead-time gates) from `config/paper_profiles.yaml`; each profile has its own $1000 bankroll in **PostgreSQL** (`polytempo_paper`). The always-on bot (`scripts/run_paper_bot.py`) opens trades at each profile's exact lead-hour gate and settles resolved events. A separate **42 active wallets** (`{model}_{trade}_active`, no lead gate) are managed by an edge-following controller that, at each lead-gate tick (the hold wallets' clock), scales into a leg while its edge holds and flattens it when the edge flips (`polytempo paper active-monitor` for a dry-run).
...
- **Reports** — every `polytempo live` run writes a Markdown report under `reports/live/`.
